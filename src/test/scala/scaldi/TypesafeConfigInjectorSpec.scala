...
class TypesafeConfigInjectorSpec extends WordSpec with Matchers {
  "TypesafeConfigInjector" should {
    ...
    "inject files" in {
      ...
      inject [List[File]] ("other.someFiles") should equal (List(new File("/var/log/foo1.log"), new File("/var/log/foo2.log")))
    }

    "inject durations" in {
      inject [Duration] ("other.timeout") should equal (Duration(10, TimeUnit.SECONDS))
      inject [List[Duration]] ("other.timeouts") should equal (List(Duration(1, TimeUnit.SECONDS), Duration(2000, TimeUnit.MILLISECONDS)))
    }

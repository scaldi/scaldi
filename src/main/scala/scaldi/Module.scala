...
/** Module is a place where you declare your bindings. It's also the most common injector that you can use in most
  * cases. It's a mutable injector so it can have a lifecycle and it also provides nice DSL for the bindings.
  *
...
trait Module
  ...
  def getBindingInternal(identifiers: List[Identifier]): Option[BindingWithLifecycle] =
    wordBindings find (_ isDefinedFor identifiers)

  /** @inheritdoc */
  def getBindingsInternal(identifiers: List[Identifier]): List[BindingWithLifecycle] =
    wordBindings filter (_ isDefinedFor identifiers)
...
@deprecated(
  "StaticModule is deprecated and will be removed soon. As an alternative you can " +
    "use `ImmutableWrapper` injector to define an immutability boundary in composition or create " +
    "your own injector that is marked as `ImmutableInjector`.",
  "0.5"
)
...
object DynamicModule {
  ...
  /** Standard factory method that accepts function that may initialize DynamicModule' bindings upon creation. It lets
    * initializing module with function instead of extending it
    *
    * {{{
    ...
    * }}}
    *
    * @param initBindingsFn
    *   function initializing DynamicModule upon creation
    * @return
    *   initialized DynamicModule
    */
...
object Args {
  ...
  /** Factory method used to initialize a `Module` with binded command line arguments
    * @param args
    *   array that will be binded to `'args` identifier
    * @return
    *   new `DynamicModule` with a binding defined for command line arguments
    */
...
object NilInjector extends ImmutableInjector {
  ...
  /** @inheritdoc */
  def getBinding(identifiers: List[Identifier]): Option[Binding] = None

  /** @inheritdoc */
  def getBindings(identifiers: List[Identifier]): List[Binding] = Nil
}
...
object SystemPropertiesInjector extends RawInjector {
  ...
  /** @inheritdoc
    * @param name
    *   system property's name
    * @return
    *   system property defined by `name`
    */
...
class PropertiesInjector private (properties: Properties) extends RawInjector {
  ...
  /** @inheritdoc
    ...
    *   property's name
    * @return
    *   property defined by `name` as instance of String
    */
...
object PropertiesInjector {
  ...
  /** Factory method to retrieve properties from a file name.
    ...
    * @return
    *   an instance of `PropertiesInjector` with properties from file with supplied file name
    */
  ...
  /** Factory method to retrieve properties from a file.
    ...
    * @return
    *   an instance of `PropertiesInjector` with properties from supplied file
    */
  ...
  /** Factory method to retrieve properties from an input stream.
    * @param stream
    *   input stream which will supply properties
    * @return
    *   an instance of `PropertiesInjector` with properties from input stream
    */
  ...
  /** Factory method to retrieve properties from a Properties instance.
    * @param properties
    *   `Properties` instance with properties
    * @return
    *   an instance of `PropertiesInjector` with properties supplied `Properties` instance
    */
...
/** Injector with bindings found in supplied typesafe configuration
  * @param config
  *   typesafe configuration to get bindings from
  */
...
object TypesafeConfigInjector {
  ...
  /** Initializes injector with the configuration found at `basename`.
    * @param baseName
    *   the base name of configuration file
    * @return
    *   initialized injector
    */
...
/** Abstract `Injector` implementation that may be extended by other injectors that get values from specific source
  * (command line, typesafe config, properties, etc.)
  */
trait RawInjector extends Injector {
  ...
  /** Retrieves bindings from cache based on supplied identifiers.
    ...
    * @return
    *   found binding, `None` if binding was not found
    */
  ...
  /** Retrieves value by name, converts it to specified type and associates it with supplied identifiers.
    ...
    * @return
    *   option with resulting binding (`None` if binding was not found by name)
    */
...
/** Container that transforms a list of bindings into a valid `Injector`
  ...
  */
class SimpleContainerInjector(bindings: Injector => List[BindingWithLifecycle])
    extends MutableInjectorUser
    with InjectorWithLifecycle[SimpleContainerInjector]
    with ShutdownHookLifecycleManager {
  lazy val preparedBindings: List[BindingWithLifecycle] = bindings(injector)

  /** @inheritdoc */
  def getBindingInternal(identifiers: List[Identifier]): Option[BindingWithLifecycle] =
    preparedBindings find (_ isDefinedFor identifiers)

  /** @inheritdoc */
  def getBindingsInternal(identifiers: List[Identifier]): List[BindingWithLifecycle] =
    preparedBindings filter (_ isDefinedFor identifiers)

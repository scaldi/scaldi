...
/** Provides injection DSL. You can extend it in your classes in order to use inject methods.
  *
  * The only responsibility of this trait is to provide you with inject function (so it just provides nice syntax for
  * injecting dependencies).
  *
  * In order to actually find and inject dependencies, `inject` method always takes an implicit parameter of type
  * `Injector`.
  *
  * Example:
  * {{{
...
trait Injectable extends Wire {
  ...
  /** Injects a dependency that was previously defined in a module
    ...
    * @param nn
    *   ensures, that type T is not inferred as `Nothing` by scala compiler, since inject needs to know the actual type
    *   at the runtime
    * @tparam T
  ...
  /** Injects a dependency defined by a constraint.
    ...
    *
    * @param constraints
    *   constraints helping to define what dependency of the type `T` to inject (typically an identifier, string or a
    *   default value)
    * @param injector
    *   implicit [[Injector]], should be defined in the scope. It's used to lookup bindings
    * @param tt
    *   ensures that meta-information if type `T` is available at the runtime
    * @param nn
    *   ensures, that type `T` is not inferred as `Nothing` by scala compiler, since inject needs to know the actual
    *   type at the runtime
    * @tparam T
    *   type of the injected dependency
    * @return
    *   instance of injected dependency
    */
  protected def inject[T](
      constraints: => InjectConstraints[T]
  ...
  /** Injects a dependency (defined by a constraint) that was previously defined as a provider in a module.
    ...
    *
    * @param constraints
    *   constraints helping to define what dependency of type `T` to inject (typically an identifier, string or a
    *   default value)
    * @param injector
    *   implicit [[Injector]], should be defined in the scope. It's used to lookup bindings
    * @param tt
    ...
    *   a new instance of injected dependency
    */
  protected def injectProvider[T](
      constraints: => InjectConstraints[T]
  )(implicit injector: Injector, tt: TypeTag[T], nn: NotNothing[T]): () => T =
  ...
  /** Inject all dependencies of type `T` that were defined in a module and that are defined by supplied identifiers.
    ...
    */
  protected def injectAllOfType[T](
      identifiers: Identifier*
  )(implicit injector: Injector, tt: TypeTag[T], nn: NotNothing[T]): List[T] =
  ...
  /** Injects a dependency defined by type `T`. If dependency is not found, initializes it with the default value from
    * the constructor of type `T`.
    *
    * This implies, that constructor must have one constructor with parameter `paramName` which has a default value.
    * Method is part of `injected` macro implementation and not intended to be used outside of it.
    *
    * Uses reflection.
    *
    * @param paramName
    ...
    */
  protected def injectWithConstructorDefault[T, C](
      paramName: String
  )(implicit injector: Injector, tt: TypeTag[T], ct: TypeTag[C]): T =
  ...
  protected def noBindingFound(identifiers: List[Identifier]): Nothing =
    throw new InjectException(
      identifiers map ("  * " +) mkString ("No binding found with following identifiers:\n", "\n", "")
    )
  ...
  /** Implicit cast of classes that can be identifiers to an `InjectConstraints` with the identified as a first element.
    ...
    *   InjectConstraints
    */
  implicit protected def canBeIdentifiedToConstraints[T: CanBeIdentifier](target: T): InjectConstraints[Nothing] =
...
trait OpenInjectable extends Injectable {
  ...
  def injectOpt[T](
      constraints: => InjectConstraints[T]
  )(implicit injector: Injector, tt: TypeTag[T], nn: NotNothing[T]): Option[T] =
    List(typeId[T]) ++ constraints.identifiers |>
      (ids => injector getBinding ids flatMap (_.get) map (_.asInstanceOf[T]) orElse constraints.default.map(_.apply()))

  override def inject[T](implicit injector: Injector, tt: TypeTag[T], nn: NotNothing[T]): T =
    super.inject[T](injector, tt, nn)

  override def inject[T](
      constraints: => InjectConstraints[T]
  )(implicit injector: Injector, tt: TypeTag[T], nn: NotNothing[T]): T =
  ...
  override def injectAllOfType[T](
      identifiers: Identifier*
  )(implicit injector: Injector, tt: TypeTag[T], nn: NotNothing[T]): List[T] =
  ...
  override def injectWithConstructorDefault[T, C](
      paramName: String
  )(implicit injector: Injector, tt: TypeTag[T], ct: TypeTag[C]): T =
  ...
  implicit override def canBeIdentifiedToConstraints[T: CanBeIdentifier](target: T): InjectConstraints[Nothing] =
...
/** Companion object of Injectable which can be just imported in contrast to Injectable trait which always need to be
  * extended. After importing it, you can use an injection DSL without extending the trait.
  *
  * Example:
  * {{{
...
class IdentifiedWord[+T](default: Option[() => T] = None, initialIdentifiers: List[Identifier] = Nil) {
  def by[I: CanBeIdentifier](target: I*): InjectConstraints[T] =
    InjectConstraints(default, initialIdentifiers ++ (target map implicitly[CanBeIdentifier[I]].toIdentifier))
}
...
case class InjectConstraints[+T](default: Option[() => T] = None, initialIdentifiers: List[Identifier] = Nil) {
  ...
  /** Helper method to define a default value for injection.
    *
    * Example: `inject [Database] ('database and by default defaultDb)`
    *
    * @param by
    *   `ByWord` that follows the "and"
    */
  def and(by: ByWord): ByWord = new ByWord(identifiers)

  /** Alias to "and"
    *
    * Example: `inject [Database] ('remote which by default defaultDb)`
    *
    * @param by
    *   ByWord that follows the "which"
    */
  def which(by: ByWord): ByWord = and(by)

  /** Alias to "and"
    *
    * Example: `inject [Database] ('remote that by default defaultDb)`
    *
    * @param by
    *   `ByWord` that follows the "that"
    */
  def that(by: ByWord): ByWord = and(by)

  /** Alias to "and"
    *
    * Example: `inject [Database] ('remote is by default defaultDb)`
    *
    * @param by
    *   `ByWord` that follows the "is"
    */
  def is(by: ByWord): ByWord = and(by)

  /** Helper method to make possible that "identified" follows "and" word.
    *
    * Example: `inject [Database] (by default defaultDb and identified by 'remote)`
    *
    * @param by
    *   `IdentifiedWord` that follows the "and"
    */

...
/** Binding is defined in a `Module` as Type -> instance relationship (sometimes with additional identifiers). I can be
  * injected in classes where implicit `Injector` is available. Binding can be defined (containing a value) or
  * undefined.
  */
trait Binding extends Identifiable {

  /** Retrieves stored binding's value, used during binding lookup during injection. If equals to `None`, binding is
...
object Binding {
  ...
  /** Standard factory method for binding that from `LifecycleManager` and `BindingWithLifecycle` creates new `Binding`
    *
    * @param lifecycleManager
    *   `LifecycleManager` that will handle initialization / destruction events for binding
    * @param binding
    *   `BindingWithLifecycle` actual binding
    * @return
    *   a new binding (without lifecycle) which delegates all method calls to the provided `binding`
    */
...
object BindingWithLifecycle {
  def apply(binding: Binding): BindingWithLifecycle = new BindingWithLifecycle {
    val lifecycle: BindingLifecycle[Any]                     = BindingLifecycle.empty
    def condition: Option[() => Condition]                   = binding.condition
    def identifiers: List[Identifier]                        = binding.identifiers
...
/** Binding that is initialized at the Injector initialization phase (normally during an application startup) and never
  * changes afterwards.
  *
  * @param createFn

...
/** Used to identify a binding. If you have two bindings of the same type (like `String` bindings), the identifiers may
  * be used to identify and to inject the right one
  */
trait Identifier {
  ...
  /** Determines if one `Identifier` is equal to another `Identifier`
    ...
    *   other `Identifier`
    * @return
    *   true if both identifiers are the same, false otherwise
    */
  def sameAs(other: Identifier): Boolean

  /** `Identifier` may be marked as required, in which case the identifier must be used in order to get the binding
    * @return
    *   true if `Identifier` is required, false otherwise
    */
...
object Identifier {
  ...
  /** Implicitly transforms a type that can be `Identifier` to an identifier
    * @param target
    *   value to be transformed to `Identifier`
    * @tparam T
    *   class that can be transformed to an `Identifier`
    * @return
    *   resulting `Identifier`
    */
  implicit def toIdentifier[T: CanBeIdentifier](target: T): Identifier =
    implicitly[CanBeIdentifier[T]].toIdentifier(target)

  /** Compares two lists of identifiers, true if actual lists at least contains all the identifiers in the desired list.
    * Also checks if all the required identifiers in the actual list are matched in the desired list
    * @param actual
    *   list of actual identifiers to compare
    * @param desired
    *   list of desired identifiers to compare
    * @return
    *   true if actual list contains at least all the identifiers in the desired list and the required actual
    *   identifiers are in desired list, false otherwise
    */
...
object CanBeIdentifier {
  ...
  /** Implementation to implicitly transform `String` into an `Identifier`. Has priority over other type class
    * implementations
    */
  ...
  implicit def ClassCanBeIdentifier[T: TypeTag]: CanBeIdentifier[Class[T]] = new CanBeIdentifier[Class[T]] {
    def toIdentifier(c: Class[T]): TypeTagIdentifier = TypeTagIdentifier.typeId[T]
  }
  ...
  implicit def TypeTagCanBeIdentifier[T: TypeTag]: CanBeIdentifier[TypeTag[T]] = new CanBeIdentifier[TypeTag[T]] {
    def toIdentifier(typeTag: TypeTag[T]): TypeTagIdentifier = TypeTagIdentifier(typeTag.tpe)
  }
...
object TypeTagIdentifier {
  ...
  /** Generates an `Identifier` from a type parameter within TypeTag
    * @tparam T
    *   `TypeTag` containing type parameter
    * @return

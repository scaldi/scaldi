...
trait Injector {
  ...
  /** Composes two injectors. Polymorphic, needs CanCompose trait implementation available in the scope
    ...
    */
  def ++[I <: Injector, R <: Injector](other: I)(implicit comp: CanCompose[this.type, I, R]): R =
    comp.compose(this, other)

  /** Composes two injectors. Note that the operands are inverted because the method starts with ":" Polymorphic, needs
    * CanCompose trait implementation available in the scope
    * @param other
    *   other injector to be composed with
    * @param comp
    *   CanCompose implementation that will handle the composition of the two injectors
    ...
    *   composed injector, depends on CanCompose trait's implementation
    */
  def ::[I <: Injector, R <: Injector](other: I)(implicit comp: CanCompose[I, this.type, R]): R =
    comp.compose(other, this)
}
...
object Injector extends LowPriorityImmutableInjectorComposition {
  ...
  /** Provides an implementation for implicit `CanCompose` for a composition between an empty injector and an another
    * empty injector
    */
  ...
  /** Provides an implementation for implicit `CanCompose` for a composition between an empty injector with a non-empty
    * injector
    */
  implicit def nilWithOtherInjectorComposition[I <: Injector]: CanCompose[NilInjector.type, I, I] =
    new CanCompose[NilInjector.type, I, I] {
      def compose(cmp1: NilInjector.type, cmp2: I): I = cmp2
    }

  /** Provides an implementation for implicit `CanCompose` for a composition between an non-empty injector and an empty
    * injector
    */
  implicit def otherInjectorWithNilComposition[I <: Injector]: CanCompose[I, NilInjector.type, I] =
    new CanCompose[I, NilInjector.type, I] {
      def compose(cmp1: I, cmp2: NilInjector.type): I = cmp1
    }
}
...
trait LowPriorityImmutableInjectorComposition extends LowPriorityMutableInjectorComposition {
  ...
  implicit def immutableComposition[I1 <: ImmutableInjector, I2 <: ImmutableInjector]
      : CanCompose[I1, I2, ImmutableInjectorAggregation] =
...
trait LowPriorityMutableInjectorComposition {
  ...
  implicit def mutableInjectorComposition[I1 <: Injector, I2 <: Injector]
      : CanCompose[I1, I2, MutableInjectorAggregation] =
    new CanCompose[I1, I2, MutableInjectorAggregation] {
      def compose(cmp1: I1, cmp2: I2) = new MutableInjectorAggregation(List(cmp1, cmp2))
    }
}

/** A very simple implementation of an injector that just delegates the binding lookup to some other injector. This will
  * protect the lifecycle of the delegated injector from any changes. It is useful in a case of a scoped binding: if you
  * want to use the injector in a composition, but don't want the composition's lifecycle to influence it. In that case,
  * before creating the composition wrap the injector into the `ImmutableWrapper`.
  * @param delegate
  *   another Injector to which delegate the binding
  */
...
object ImmutableWrapper {
  ...
  /** Standard factory method for Immutable Wrapper. All binding lookup will be delegated to the injector provided in
    * the parameters
    * @param delegate
    *   injector to which delegate binding lookup
    * @return
    *   ImmutableWrapper
    */
...
trait CanCompose[-A, -B, +R] {
  ...
  /** Composes two injectors
    * @param cmp1
    *   first injector in the composition
    * @param cmp2
    *   second injector in the composition
    * @return
    *   composed injector
    */
...
class ImmutableInjectorAggregation(chain: List[ImmutableInjector]) extends Injector {
  ...
  /** @inheritdoc */
  def getBinding(identifiers: List[Identifier]): Option[Binding] =
    chain.view.map(_ getBinding identifiers).collectFirst { case Some(b) => b }
...
/** The result of the composition of two or more of the mutable injectors
  ...
  *   currently composed immutable injectors
  */
class MutableInjectorAggregation(chain: List[Injector])
    extends InjectorWithLifecycle[MutableInjectorAggregation]
  ...
  /** Initialize all injectors in the composition by setting their parent injector to the one passed in the parameters.
    * @param newParentInjector
    *   the injector which will be parent injector for composed injectors
    */
...
/** Contains implicit reference to injector: the final injector composition which is used by inject. Injector
  * aggregation will set it during the initialization phase
  */
...
/** Trait for injectors that have lifecycle
  ...
  *   type for initializeable injector
  */
trait InjectorWithLifecycle[I <: InjectorWithLifecycle[I]]
    extends Injector
    with Initializeable[I]
    with MutableInjector {
  this: I with LifecycleManager with Initializeable[I] =>

  /** @inheritdoc */
  final def getBinding(identifiers: List[Identifier]): Option[Binding] =
    initNonLazy() |> (_ getBindingInternal identifiers map (Binding.apply(this, _)))

  /** @inheritdoc */
  final def getBindings(identifiers: List[Identifier]): List[Binding] =
    initNonLazy() |> (_ getBindingsInternal identifiers map (Binding.apply(this, _)))
...
/** Custom exception for injectors
  * @param message
  *   String message describing the problem
  */
